...
def _extract_clip_batch(
    ...
    Writes a batch parquet with embedded image bytes directly to the volume.
    Returns only lightweight metadata (no image data through RPC).
    """
    import os
    import shutil
    import tempfile
    ...
    if samples:
        batch_dir = f"{CACHE_DIR}/extracted/{scale}/batches"
        os.makedirs(batch_dir, exist_ok=True)
        df = pd.DataFrame(samples)
        df.to_parquet(f"{batch_dir}/batch_{batch_idx:04d}.parquet", index=False)
    ...
    return {"meta": meta, "errors": errors, "n_samples": len(samples)}
...
def extract_parallel(
    ...
    # Resume: check which batch parquets already exist
    ...
    for f in os.listdir(batch_dir):
        if f.startswith("batch_") and f.endswith(".parquet"):
            ...
            existing_batches.add(idx)
    if existing_batches:
        print(f"Resuming: {len(existing_batches)} batch parquets already exist")

    pending = [(i, b) for i, b in enumerate(batches) if i not in existing_batches]
    pending_batches = [b for _, b in pending]
    pending_indices = [i for i, _ in pending]
    print(f"Pending: {len(pending)} batches of {n_batches} total")

    print(f"Launching {len(pending)} parallel workers ({batch_size} clips each)...")
    t_start = time.time()

    all_meta = list(prev_samples)
    all_errors = []
    ...
    for r in _extract_clip_batch.map(
        pending_batches,
        [eval_clips] * len(pending),
        [scale] * len(pending),
        pending_indices,
        [preloaded] * len(pending),
    ):
        all_meta.extend(r["meta"])
        all_errors.extend(r["errors"])
        total_new_samples += r["n_samples"]
        completed += 1

        if completed % 5 == 0:
            print(f"  Progress: {completed}/{n_batches} batches, {total_new_samples} new samples")

    # Write metadata-only parquet (no image bytes — lightweight for inspection)
    ...
    print(f"\nBatch parquets with embedded images at: {batch_dir}/")
    return len(df)
